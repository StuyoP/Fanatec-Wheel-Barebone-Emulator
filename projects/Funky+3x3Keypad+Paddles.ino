...
const byte ROWS = 3; //ROWs for Keypad
const byte COLS = 3; //Colums for Keypad
...
const byte rowPins[ROWS] = { A0, A1, A2}; //connect to the row pinouts of the keypad
const byte colPins[COLS] = { A3, A4, A5}; //connect to the column pinouts of the keypad
  // Create the Keypad
...
#ifdef  HAS_FUNKY
    pinMode(FunkyUP, INPUT_PULLUP);
    pinMode(FunkyLEFT, INPUT_PULLUP);
    pinMode(FunkyRIGHT, INPUT_PULLUP);
    pinMode(FunkyDOWN, INPUT_PULLUP);	
    pinMode(FunkyOK, INPUT_PULLUP);
#endif
...
#ifdef HAS_FUNKY
  int FunkyVal = 0;
  if (!digitalRead(FunkyOK)){ FunkyVal = 1;}
  if (!digitalRead(FunkyUP)){ FunkyVal = 2;}
  if (!digitalRead(FunkyLEFT)){ FunkyVal = 3;}
  if (!digitalRead(FunkyRIGHT)){ FunkyVal = 4;}
  if (!digitalRead(FunkyDOWN)){ FunkyVal = 5;} 
  
  if (FunkyVal != prevFunkyVal) {   // only if button has changed, do something
    Serial.print("Funky button:");
    Serial.println(FunkyVal); 
    FunkyBitChange(prevFunkyVal, false);  // turn the previous Funky button bit off.
    FunkyBitChange(FunkyVal, true);   // turn the current Funky button bit on.   
  }
  prevFunkyVal = FunkyVal;
#endif
...
void FunkyBitChange(uint8_t FunkyVal, bool bitOn) {
